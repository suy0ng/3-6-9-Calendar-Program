...
void add_Schedule()
{
	...
	char add_answer; //실행을 할수있게 하는 변수 선언
	gotoxy(2, 33); // 출력 이동
	...
	else if (add_answer == 'y' || add_answer == 'Y')
	{
		...
		{
			printf("    날짜를 다시확인 부탁드립니다."); //날짜를 다시확인을 부탁하는 안내문 출력
			system(500); //0.5초 대기
			printf("\n     입력창을 종료합니다. 입력창을 다시 실행시켜주십시오. \n"); //입력창을 종료시킨다고 안내문을 출력한다
			Sleep(500); //0.5초 대기
			printf("\n      입력창을 종료합니다. \n"); //입력창 종료 전 안내문 출력
...
void draw_Calendar() {
    ...
    while (1) {
        ...
        if (q == 's' || q == 'S') {
            oneweek--;
            if ((year % 400 == 0 || (year % 100 != 0 && year % 4 == 0)) && month > 2 || (year % 400 == 0 || (year % 100 != 0 && year % 4 == 0)) && month < 3)
                oneweek--;
            year--;
