...
void view_month_Schedule(int year,int month );
...
void view_month_Schedule(int year, int month)
{
	...
	{
		if (schedule[year - 2022][month][day][0] != NULL) //일정이 비어있지않는다면
		{
			if (Not_Schedule != 6) //최대 6개 출력가능
			{
				printf("\n\t   %d 일 : %s\n", day, schedule[year - 2022][month][day]); // 일정 출력
...
void draw_Calendar() 
{
    ...
    while (1) {
        ...
        if (q == 'v' || q == 'V')
        {
           view_month_Schedule(year, month);
        }
